...
vim.lsp.config("bash-language-server", {
    capabilities = capabilities, -- variable capabilities is 
                                -- require("cmp_nvim_lsp").default_capabilities()
    cmd = { 
        vim.fn.exepath("bash-language-server"), 
        "start" 
    }, -- you must know how to activate the lsp,
       -- like bash-language-server activated it with "bash-language-server start" 

    filetypes = { -- set filetypes, configuration for when lsp will be active
        "sh",
        "bash",
        "zsh",
    },
